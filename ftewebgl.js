        ...
        for (let i = 0; i < names.length; i++) {
            ...
            if (typeof ab == "string") {
                ...
                xhr.onprogress = function(e) {
                    if (Module["setStatus"]) Module["setStatus"](n + " (" + e.loaded + "/" + e.total + ")")
                };
...
if (typeof man != "undefined") Module["arguments"] = Module["arguments"].concat(["-manifest", man]);
...
function createWasm() {
    ...
    function receiveInstance(instance, module) {
        ...
        Module["asm"] = exports;
        wasmMemory = Module["asm"]["nc"];
        updateGlobalBufferAndViews(wasmMemory.buffer);
        wasmTable = Module["asm"]["pc"];
        addOnInit(Module["asm"]["oc"]);
...
function _emscriptenfte_async_wget_data2(url, ctx, onload, onerror, onprogress) {
    ...
    try {
        ...
        if (onerror) getWasmTableEntry(onerror)(ctx, 404);
        return
    }
    http.responseType = "arraybuffer";
    http.onload = function(e) {
        if (http.status == 200) {
            if (onload) getWasmTableEntry(onload)(ctx, _emscriptenfte_buf_createfromarraybuf(http.response))
        } else {
            if (onerror) getWasmTableEntry(onerror)(ctx, http.status)
        }
    };
    http.onerror = function(e) {
        if (onerror) getWasmTableEntry(onerror)(ctx, 0)
    };
    http.onprogress = function(e) {
        if (onprogress) getWasmTableEntry(onprogress)(ctx, e.loaded, e.total)
    };
    try {
        http.send(null)
    } catch (e) {
...
function _emscriptenfte_polljoyevents() {
    ...
    if (gamepads !== undefined)
        for (var i = 0; i < gamepads.length; i += 1) {
            ...
            for (var j = 0; j < gp.axes.length; j += 1) getWasmTableEntry(FTEC.evcb.jaxis)(gp.index, j, gp.axes[j], gp.mapping == "standard")
        }
}
...
var FTEC = {
    ...
    handleevent: function(event) {
        switch (event.type) {
            ...
            case "mouseout":
                if (FTEC.evcb.button != 0) {
                    for (var i = 0; i < 8; i++) getWasmTableEntry(FTEC.evcb.button)(0, false, i)
                }
            ...
            case "keyup":
                if (FTEC.evcb.key != 0 && event.keyCode != 122) {
                    ...
                    if (getWasmTableEntry(FTEC.evcb.key)(0, event.type == "keydown", event.keyCode, codepoint)) event.preventDefault()
                }
                break;
            case "touchstart":
            case "touchend":
            ...
            case "touchmove":
                ...
                for (var i = 0; i < touches.length; i++) {
                    var t = touches[i];
                    if (FTEC.evcb.mouse) getWasmTableEntry(FTEC.evcb.mouse)(t.identifier + 1, true, t.pageX, t.pageY, 0, Math.sqrt(t.radiusX * t.radiusX + t.radiusY * t.radiusY));
                    if (FTEC.evcb.button) {
                        if (event.type == "touchstart") getWasmTableEntry(FTEC.evcb.button)(t.identifier + 1, 1, -1);
                        else if (event.type != "touchmove") getWasmTableEntry(FTEC.evcb.button)(t.identifier + 1, 0, -1)
                    }
                }
                break;
            case "dragenter":
            ...
            case "gamepaddisconnected":
                ...
                if (FTEC.evcb.jaxis)
                    for (var j = 0; j < 6; j += 1) getWasmTableEntry(FTEC.evcb.jaxis)(gp.index, j, 0, true);
                if (FTEC.evcb.jbutton)
                    for (var j = 0; j < 32 + 4; j += 1) getWasmTableEntry(FTEC.evcb.jbutton)(gp.index, j, 0, true);
...
function _emscriptenfte_setupcanvas(nw, nh, evresize, evmouse, evmbutton, evkey, evfile, evcbufadd, evjbutton, evjaxis, evwantfullscreen) {
    try {
        ...
        window.onresize = function() {
            ...
            if (FTEC.evcb.resize != 0) getWasmTableEntry(FTEC.evcb.resize)(Module["canvas"].width, Module["canvas"].height)
        };
        window.onresize();
...
var asmLibraryArg = {
    "W": _Sys_Clipboard_PasteText,
    "n": _Sys_SaveClipboard,
    ...
    "V": _alGetString,
    "$b": _alIsBuffer,
    "Va": _alIsExtensionPresent,
    "Sa": _alIsSource,
    "ac": _alListenerf,
    "U": _alListenerfv,
    "Ta": _alSourcePlay,
    "T": _alSourceQueueBuffers,
    "Ra": _alSourceStop,
    "Ua": _alSourceUnqueueBuffers,
    "m": _alSourcef,
    "Qa": _alSourcefv,
    "u": _alSourcei,
    "Ob": _alcCaptureCloseDevice,
    "Pb": _alcCaptureOpenDevice,
    "Lb": _alcCaptureSamples,
    "Nb": _alcCaptureStart,
    "Mb": _alcCaptureStop,
    "ra": _alcCloseDevice,
    "Wa": _alcCreateContext,
    "Za": _alcDestroyContext,
    "Sb": _alcGetContextsDevice,
    "Tb": _alcGetCurrentContext,
    "Qb": _alcGetEnumValue,
    "Rb": _alcGetError,
    "qa": _alcGetIntegerv,
    "A": _alcGetString,
    "Xa": _alcIsExtensionPresent,
    "sa": _alcMakeContextCurrent,
    "Ya": _alcOpenDevice,
    "Vb": _alcProcessContext,
    "Ub": _alcSuspendContext,
    "Kb": _emscripten_alcDevicePauseSOFT,
    "Jb": _emscripten_alcDeviceResumeSOFT,
    "Ib": _emscripten_alcGetStringiSOFT,
    "Hb": _emscripten_alcResetDeviceSOFT,
    "Wb": _emscripten_asm_const_int,
    "Gb": _emscripten_console_error,
    "Pa": _emscripten_get_now,
    "Fb": _emscripten_memcpy_big,
    "yb": _emscripten_resize_heap,
    "Xb": _emscripten_run_script_string,
    "N": _emscriptenfte_abortmainloop,
    "Zb": _emscriptenfte_al_loadaudiofile,
    "bb": _emscriptenfte_alert,
    "cb": _emscriptenfte_async_wget_data2,
    "X": _emscriptenfte_buf_create,
    "lc": _emscriptenfte_buf_delete,
    "dc": _emscriptenfte_buf_getsize,
    "cc": _emscriptenfte_buf_open,
    "bc": _emscriptenfte_buf_pushtolocalstore,
    "fc": _emscriptenfte_buf_read,
    "gc": _emscriptenfte_buf_release,
    "kc": _emscriptenfte_buf_rename,
    "ec": _emscriptenfte_buf_write,
    ...
    "hc": _emscriptenfte_setupmainloop,
    "fb": _emscriptenfte_updatepointerlock,
    "$a": _emscriptenfte_uptime_ms,
    "ic": _emscriptenfte_window_location,
    "y": _emscriptenfte_ws_close,
    "wa": _emscriptenfte_ws_connect,
    "Y": _emscriptenfte_ws_recv,
    "H": _emscriptenfte_ws_send,
    "jc": _emscritenfte_buf_enumerate,
    "zb": _environ_get,
...
var ___wasm_call_ctors = Module["___wasm_call_ctors"] = function() {
    return (___wasm_call_ctors = Module["___wasm_call_ctors"] = Module["asm"]["oc"]).apply(null, arguments)
};
var _malloc = Module["_malloc"] = function() {
    return (_malloc = Module["_malloc"] = Module["asm"]["qc"]).apply(null, arguments)
};
var _main = Module["_main"] = function() {
    return (_main = Module["_main"] = Module["asm"]["rc"]).apply(null, arguments)
};
var _emscripten_builtin_malloc = Module["_emscripten_builtin_malloc"] = function() {
    return (_emscripten_builtin_malloc = Module["_emscripten_builtin_malloc"] = Module["asm"]["sc"]).apply(null, arguments)
};
var _emscripten_builtin_free = Module["_emscripten_builtin_free"] = function() {
    return (_emscripten_builtin_free = Module["_emscripten_builtin_free"] = Module["asm"]["tc"]).apply(null, arguments)
};
var stackAlloc = Module["stackAlloc"] = function() {
    return (stackAlloc = Module["stackAlloc"] = Module["asm"]["uc"]).apply(null, arguments)
};
var calledRun;
